...
//    DisplayTest();
    const char test_string[16] = "test";
    DisplayText(1U, test_string, strlen(test_string), 0U);
    DisplayRefresh();
//    printf("got here");
...
//
    while(1){
        ...
        if (CarSteer > DEADZONE){
        	strcpy(direction_string, "left");
        } else if (CarSteer < -DEADZONE) {
        	strcpy(direction_string, "right");
        } else {
        	strcpy(direction_string, "center");
        }
